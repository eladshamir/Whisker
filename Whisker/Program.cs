...
namespace Whisker
{
  ...
  /path:<PATH>        Optional.  Set the path to store the generated self-siged certificate for authentication. If not 
  provided, the certificate will be printed as a Base64 blob.
  /password:<PASWORD>       Optional. Set the password for the stored self-siged certificate. If not provided, a 
  ...
  /target:<samAccountName>      Required. Set the target name. Computer objects should end with a '$' sign.
  /deviceID:<GUID>       Required. Set the DeviceID of the value to remove from the attribute msDS-KeyCredentialLink of
  the target object. Must be a valid GUID.  
  /domain:<FQDN>      Optional. Set the target Fully Qualified Domain Name (FQDN). If not provided, will try to
  resolve the FQDN of the current user.
  /dc:<IP/HOSTNAME>   Optional.        Set the target Domain Controller (DC). If not provided, will try to target the
  Primary Domain Controller (PDC).
  ...
  Warning: Clearing the msDS-KeyCredentialLink attribute of accounts configured for passwordless authentication
  ...
  For this attack to succeed, the environment must have a Domain Controller running on Windows Server 2016,
